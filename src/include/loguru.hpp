...
#undef MAX
	...
	*/
	...
	void logit(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, fmt::ArgList args);
	...
	// Actual logging function. Use the LOG macro instead of calling this directly.
	void logit(Verbosity verbosity, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, ...) LOGURU_PRINTF_LIKE(4, 5);
...
#define VLOG_F(verbosity, ...)                                                                     \
	((verbosity) > loguru::current_verbosity_cutoff()) ? (void)0                                   \
									  : loguru::logit(verbosity, __FILE__, __LINE__, __VA_ARGS__)
...
#define VTSEQ(ID) ("\e[" #ID "m")
...
#if LOGURU_USE_FMTLIB
	void logit(Verbosity verbosity, const char* file, unsigned line, const char* format, fmt::ArgList args)
	{
...
#else
	void logit(Verbosity verbosity, const char* file, unsigned line, const char* format, ...)
	{
		va_list vlist;
		va_start(vlist, format);
	...
	StreamLogger::~StreamLogger() noexcept(false)
	{
		...
		logit(_verbosity, _file, _line, "%s", message.c_str());
	}
